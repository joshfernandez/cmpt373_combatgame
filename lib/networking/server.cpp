...
public:
    ...
    static constexpr unsigned BUFFER_SIZE = 9000;
...
void Server::Channel::start() {
    readLine();
}


void Server::Channel::disconnect() {
    disconnected = true;
    socket.cancel();
    socket.close();
}
...
void Server::Channel::send(std::string outgoing) {
    if (outgoing.empty()) {
        return;
    }
